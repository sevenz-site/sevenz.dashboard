-- Exposes movements.plazo_dias through get_shared_balance() so the public
-- client screen (/s/[token]) can show "Plazo de pago" in its movement
-- detail popover. Does NOT expose photo_path — attachment photos stay
-- owner-only, never signed for anonymous callers.
--
