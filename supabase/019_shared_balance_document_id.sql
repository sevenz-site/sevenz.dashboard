-- Additive migration: run once in the SQL editor (both dev and production
-- Supabase projects). Doesn't touch existing data.
-- Exposes clients.document_id through get_shared_balance() so the public
-- client screen (/s/[token]) can show it under the client's name.
--
-- Safe to CREATE OR REPLACE unlike the client_summary view fix: this
-- function returns a single json blob (not ordinal table columns), so
-- adding a new key has no ordering constraint.
